...
const stream = util.promisify(T.stream('statuses/filter', params).on);
          ...
          T.post('statuses/retweet/:id', {id: id.toString()})
            .then(() => {
              ...
              T.post('/favorites/create', {id: id.toString()})
                ...
                .catch(() => {
                  console.error(`Couldn't favorite the tweet with id: ${id}`);
                });
            })
            .catch(() => {
              console.error(`Couldn't retweet the tweet with id: ${id}`);
            })
        }
      }
    }
